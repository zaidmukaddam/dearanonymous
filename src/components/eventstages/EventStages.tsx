...
import { loginNmims } from "gateways/AuthGateway";
  ...
  const incrementStage = () => {
    ...
    if (
      ...
      if (stageTwoLocation.length > 65) {
        toast.error(
          `Event location is ${stageTwoLocation.length - 65
          } characters too long`
        );
      } else if (stageTwoDescription.length > 800) {
        toast.error(
          `Event description is ${stageTwoDescription.length - 800
          } characters too long`
        );
  ...
  if (!user) {
    return (
      <div className={styles.NotLoggedIn}>
        ...
        <p>
          Dear Anonymous does not store any personal information about the creators
          of events unless an optional contact email is provided. Logging in is
          only required so that we can verify that you are a NMIMS student or
          community member.
        </p>
        <button onClick={loginNmims} className={styles.LoginButton}>
          Log in here
        </button>
      </div>
    );
