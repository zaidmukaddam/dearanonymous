...
export function FeedPicker({
  ...
  const moveUnderline = useCallback(() => {
    if (underlineRef.current && underlinedRef.current) {
      ...
      underlineRef.current.style.left = `${underlinedRef.current.offsetLeft + leftAdjust
        }px`;
      underlineRef.current.style.width = `${underlinedRef.current.offsetWidth + widthAdjust
        }px`;
    }
